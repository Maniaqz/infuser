package com.classfuse;

import com.classfuse.annotations.Component;
import com.classfuse.utils.InfuserUtil;
...
/**
 * Infuser, to create objects for all @CustomService classes. autowire/inject
 * all dependencies
 */
public class Infuser {
    ...
    private static Infuser infuser;
    ...
    public static void startApplication(Class<?> mainClass) {
        try {
            synchronized (Infuser.class) {
                if (infuser == null) {
                    infuser = new Infuser();
                    infuser.initFramework(mainClass);
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public static <T> T getService(Class<T> classz) {
        try {
            return infuser.getBeanInstance(classz);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private void initFramework(Class<?> mainClass)
            throws InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, NoSuchMethodException {
        ...
        try (ClassHunter.SearchResult result = classHunter.findBy(
            ...
            for (Class<?> classz : classes) {
                if (classz.isAnnotationPresent(Component.class)) {
                    ...
                    InfuserUtil.autowire(this, classz, classInstance);
                }
            }
